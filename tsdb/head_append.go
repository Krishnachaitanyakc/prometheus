...
func (s *memSeries) appendHistogram(st, t int64, h *histogram.Histogram, appendID uint64, o chunkOpts) (sampleInOrder, chunkCreated bool) {
	...
	// Hand the current appender (whichever concrete chunk type it happens to
	// be) to the new appender as 'prev' so it can compute the counter-reset
	// header across the chunk boundary. AppendHistogram does the concrete
	// type check on the path that actually needs it.
	prevApp := s.app
...
func (s *memSeries) appendFloatHistogram(st, t int64, fh *histogram.FloatHistogram, appendID uint64, o chunkOpts) (sampleInOrder, chunkCreated bool) {
	...
	// Hand the current appender (whichever concrete chunk type it happens to
	// be) to the new appender as 'prev' so it can compute the counter-reset
	// header across the chunk boundary. AppendFloatHistogram does the
	// concrete type check on the path that actually needs it.
	prevApp := s.app
